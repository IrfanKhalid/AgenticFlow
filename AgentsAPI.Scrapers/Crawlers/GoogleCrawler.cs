        ...
        public static async Task<List<JobDetail>> CrawlGoogleAsync(IBrowserContext browser)
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    // Check if "Next page" button exists and is enabled await page.Locator("button[aria-label='Next jobs']")
                    var nextButton = page.Locator("a[aria-label='Go to next page']");
                    if (await nextButton.CountAsync() == 0)
                        break;

                    var isDisabled = await nextButton.Last.IsDisabledAsync();
                    if (isDisabled)
                        break;

                    // Click next page and wait for the listing to refresh
                    await nextButton.Last.ClickAsync();
